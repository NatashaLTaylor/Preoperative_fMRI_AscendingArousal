...
# include color limits to ensure same scale across all plots
plot_brain_data_17nets <- function(data, filename, color_limits = NULL) {
  library(ggplot2)
  library(ggseg)
  library(ggsegSchaefer)
  library(dplyr)
  ...
  if (!is.null(color_limits)) {
      color_scale <- scale_fill_gradient2(
        ...
        midpoint = 0, 
        limits = color_limits,
        oob = scales::squish   # <-- KEY FIX: clamps out-of-bounds values instead of censoring
      )
  } else {
      color_scale <- scale_fill_gradient2(
        low = "#C03B2B", 
...
plot_brain_17nets_blueorange_alt <- function(data, filename) {
  library(ggplot2)
  library(ggseg)
  library(ggsegSchaefer)
  library(dplyr)
  ...
  p <- ggplot(data_plot, aes(fill = SignificantCoefficient)) +
    geom_brain(atlas = schaefer17_400, show.legend = TRUE) +
    #scale_fill_gradient2(low = "#CA1C6A", mid = "white", high = "#173F6C", midpoint = 0) +
    scale_fill_gradient2(low = "#7fb6cc", mid = "white", high =   "#f9a340", midpoint = 0) +
    theme_void() +
    theme(legend.position = "bottom",
          legend.text = element_text(size = 12),
          legend.title = element_text(size = 14),
...
# Updated PC Analysis plots
```{r}
# load in regional names
schaef_names <- read.csv("/Users/ntaylor/Library/CloudStorage/OneDrive-TheUniversityofSydney(Staff)/Postdoc_Rob/Analysis/Graph_Theory/schaef_400/lm_model/schaef_names.csv", header = TRUE, stringsAsFactors = FALSE)
regions = schaef_names$ROI.Name
label <- schaef_names$ROI.Name
...
data <- read.csv('mean_diff_del_health_pc_avg_time.csv',header=FALSE)
# Create a data frame for plotting
data_plot <- data.frame(label = label[1:400], SignificantCoefficient = data$V1[1:400])
...
setwd('/Users/ntaylor/Library/CloudStorage/OneDrive-TheUniversityofSydney(Staff)/Postdoc_Rob/Analysis/Graph_Theory/schaef_400/csf_regress_redo')
# Calculate FC differences by whole cortex permutation test (edge-wise) & look at significantly different avg differences 
data <- read.csv('nbM_combined_fc_sig_diff_csf4thvent.csv',header=FALSE)
data_plot <- data.frame(label = label[1:400], SignificantCoefficient = data$V1[1:400])
plot_brain_data_17nets(data_plot, "nbM_combined_fc_sig_diff_csf4thvent")

data <- read.csv('lc_combined_fc_sig_diff_csf4thvent.csv',header=FALSE)
data_plot <- data.frame(label = label[1:400], SignificantCoefficient = data$V1[1:400])
plot_brain_data_17nets(data_plot, "lc_combined_fc_sig_diff_csf4thvent")



# Permutation just on the LC  & nbM connectivity profiles: 
data <- read.csv('nbM_combined_fc_sig3_diff_csf4thvent.csv',header=FALSE)
data_plot <- data.frame(label = label[1:400], SignificantCoefficient = data$V1[1:400])
plot_brain_data_17nets(data_plot, "nbM_combined_fc_sig3_diff_csf4thvent")



# write the final FC AAS plots
data <- read.csv('lc_combined_fc_sig_plot.csv',header=FALSE)
data_plot <- data.frame(label = label[1:400], SignificantCoefficient = data$V1[1:400])
plot_brain_data_17nets(data_plot, "lc_combined_fc_sig_diff_csf4thvent_updated2")

data <- read.csv('nbm_combined_fc_sig_plot.csv',header=FALSE)
data_plot <- data.frame(label = label[1:400], SignificantCoefficient = data$V1[1:400])
plot_brain_data_17nets(data_plot, "nbm_combined_fc_sig_diff_csf4thvent_updated2")
...
Updated_setwd=('/Users/ntaylor/Library/CloudStorage/OneDrive-TheUniversityofSydney(Staff)/Postdoc_Rob/Analysis/Graph_Theory/schaef_400/csf_regress_redo/')

#select working directory
setwd(Updated_setwd)

#load the data
data1 <- read.csv('mean_sig_mtd_lcavg.csv', header= FALSE)
data2 <- read.csv('mean_sig_mtd_lcavg_thresholded.csv', header= FALSE)
data3 <- read.csv('mean_sig_mtd_lcavg_alltimepoints.csv', header= FALSE)
data4 <- read.csv('mean_sig_mtd_lcavg_thresholdfirst.csv', header= FALSE)
...
data_plot3 <- data.frame(label = label[1:400], SignificantCoefficient = data3$V1[1:400])
#data_plot4 <- data.frame(label = label[1:400], SignificantCoefficient = data4$V1[1:400])
# Print the data frame to verify
#View(data_plot1)
#View(data_plot2)
#View(data_plot3)
...
#plot onto the brain
#setwd('/Users/ntaylor/Library/CloudStorage/OneDrive-TheUniversityofSydney(Staff)/Postdoc_Rob/Analysis/Graph_Theory/schaef_400/')
plot_brain_data_17nets(data_plot1, "mean_sig_permuted_diff_mtd_lcavg_overtime_updated")
plot_brain_data_17nets(data_plot2, "mean_sig_permuted_thresholded_diff_mtd_lcavg_overtime_updated") #max 0.37 and min -0.31
plot_brain_data_17nets(data_plot3, "mean_sig_permuted_diff_mtd_lcavg_alltimepoints_updated")

#plot_brain_data_17nets(data_plot4, "mean_sig_permuted_diff_mtd_lcavg_thresholdfirst_updated")

plot_brain_data_17nets(data_plot2, "mean_sig_permuted_thresholded_diff_mtd_lcavg_overtime_updated2", color_limits = c(-0.35, 0.35)) 


data_plot2_threshold <- data.frame(
  label = label[1:400],
  SignificantCoefficient = ifelse(abs(data2$V1[1:400]) < 0.1, 0, data2$V1[1:400])
)
plot_brain_data_17nets(data_plot2_threshold, "mean_sig_permuted_thresholded_diff_mtd_lcavg_overtime_updated3", color_limits = c(-0.4, 0.4)) 
...
data1 <- read.csv('mean_sig_mtd_nbmavg.csv', header= FALSE)
data2 <- read.csv('mean_sig_mtd_nbmavg_thresholded.csv', header= FALSE)
data3 <- read.csv('mean_sig_mtd_nbmavg_alltimepoints.csv', header= FALSE)
data4 <- read.csv('mean_sig_mtd_nbmavg_thresholdfirst.csv', header= FALSE)
# load in regions names
schaef_names <- read.csv("/Users/ntaylor/Library/CloudStorage/OneDrive-TheUniversityofSydney(Staff)/Postdoc_Rob/Analysis/Graph_Theory/schaef_400/lm_model/schaef_names.csv", header = TRUE, stringsAsFactors = FALSE)
regions = schaef_names$ROI.Name
label <- schaef_names$ROI.Name
...
#plot onto the brain
#setwd('/Users/ntaylor/Library/CloudStorage/OneDrive-TheUniversityofSydney(Staff)/Postdoc_Rob/Analysis/Graph_Theory/schaef_400/')
plot_brain_data_17nets(data_plot1, "mean_sig_permuted_diff_mtd_nbmavg_overtime_updated")
plot_brain_data_17nets(data_plot2, "mean_sig_permuted_thresholded_diff_mtd_nbmavg_overtime_updated") #max 0.37 and min -0.31
plot_brain_data_17nets(data_plot3, "mean_sig_permuted_diff_mtd_nbmavg_alltimepoints_updated")
plot_brain_data_17nets(data_plot4, "mean_sig_permuted_diff_mtd_nbmavg_thresholdfirst_updated") #min -0.3 max 0.37

# thresholding the plots
data_plot2_threshold <- data.frame(
  label = label[1:400],
  SignificantCoefficient = ifelse(abs(data2$V1[1:400]) < 0.1, 0, data2$V1[1:400])
)
plot_brain_data_17nets(data_plot2_threshold, "mean_sig_permuted_diff_mtd_nbmavg_overtime_updated_thresholded", color_limits = c(-0.35, 0.35)) #max 0.34 min -0.3

plot_brain_data_17nets(data_plot2, "mean_sig_permuted_thresholded_diff_mtd_nbmavg_overtime_updated2", color_limits = c(-0.35, 0.35)) #max 0.34 min -0.3

plot_brain_data_17nets(data_plot2_threshold, "mean_sig_permuted_thresholded_diff_mtd_nbmavg_overtime_updated3", color_limits = c(-0.4, 0.4)) #max 0.34 min -0.3
...
for (time_point_idx in 1:5) {
  ...
  data_file <- paste0('grp_diff_timepoint_', time_point_idx, '_mtd_lc.csv')
  ...
  plot_name <- paste0("mean_diff_selecttimepoint", time_point_idx, "_lc_lda")
...
# Create the data frame for plotting
  ...
  # Generate a unique name for the plot
  # Set working directory for saving plots LDA_separated_diff_del_nondel_like_avg_dFC_mtd_nbm.csv
setwd('/Users/ntaylor/Library/CloudStorage/OneDrive-TheUniversityofSydney(Staff)/Postdoc_Rob/Analysis/Graph_Theory/schaef_400/PCA_LDA/Python/')
...
```
## REDO LDA SEPARATED PLOTS ON UDPATED ANALYSIS WITH CSF REGRESSING - FIGURE 4
```{r}
schaef_names <- read.csv("/Users/ntaylor/Library/CloudStorage/OneDrive-TheUniversityofSydney(Staff)/Postdoc_Rob/Analysis/Graph_Theory/schaef_400/lm_model/schaef_names.csv", header = TRUE, stringsAsFactors = FALSE)
regions <- schaef_names$ROI.Name
label <- schaef_names$ROI.Name
...
## nbM dFC LDA separate plots:
# Plot for Fig. 4 A-B - LDA coefficient for group separation for LC dFC
data <- read.csv('lda_coefficient_weights_overtime_mtd_nbm.csv', header = TRUE)
data_plot <- data.frame(label = label[1:400], SignificantCoefficient = data$X0[1:400])

#plot_brain_17nets_blueorange(data_plot, plot_name)
plot_brain_17nets_blueorange(data_plot, "lda_coefficient_weights_overtime_mtd_nbm_updated")





## Selected Maximally Separating Timepoint from LDA - dFC coefficients plotted
#LDA_coeffs_weights_select_time_delta_avg_dFC_mtd_nbm.csv
data <- read.csv('LDA_coeffs_weights_select_time_delta_avg_dFC_mtd_nbm.csv', header = TRUE)
plot_brain_data_17nets(data_plot, "LDA_coeffs_weights_select_time_delta_avg_dFC_mtd_nbm_updated2")
...
# Maximally Separated Groups - mean difference in the patterns for delirium and non-delirious 
# LDA_separated_diff_del_nondel_like_avg_dFC_mtd_nbm.csv

data <- read.csv('LDA_separated_diff_del_nondel_like_avg_dFC_mtd_nbm.csv',header=TRUE)
data_plot <- data.frame(label = label[1:400], SignificantCoefficient = data$X0[1:400])

plot_brain_data_17nets(data_plot,'LDA_separated_diff_del_nondel_like_avg_dFC_mtd_nbm_updated')


# Data for each group
data <- read.csv('LDA_coeffs_weights_select_time_delirium_avg_dFC_mtd_nbm.csv', header = TRUE)
data_plot <- data.frame(label = label[1:400], SignificantCoefficient = data$X0[1:400])
plot_brain_data_17nets(data_plot,'LDA_coeffs_weights_select_time_delirium_avg_dFC_mtd_nbm_updated')

data <- read.csv('LDA_coeffs_weights_select_time_nondelirium_avg_dFC_mtd_nbm.csv', header = TRUE)
data_plot <- data.frame(label = label[1:400], SignificantCoefficient = data$X0[1:400])
plot_brain_data_17nets(data_plot,'LDA_coeffs_weights_select_time_nondelirium_avg_dFC_mtd_nbm_updated')


## LC dFC LDA separate plots:
# Plot for Fig. 4 A-B - LDA coefficient for group separation for LC dFC
data <- read.csv('lda_coefficient_weights_overtime_mtd_lc.csv', header = TRUE)
data_plot <- data.frame(label = label[1:400], SignificantCoefficient = data$X0[1:400])
plot_brain_17nets_blueorange_alt(data_plot, "lda_coefficient_weights_overtime_mtd_lc_updated")

# Plots the selected differences at the maximally separating time point for LC dFC
data <- read.csv('LDA_coeffs_weights_select_time_delta_avg_dFC_mtd_lc.csv', header = TRUE)
data_plot <- data.frame(label = label[1:400], SignificantCoefficient = data$Delta[1:400])
plot_brain_data_17nets(data_plot, "LDA_coeffs_weights_select_time_delta_avg_dFC_mtd_lc_updated")

# LDA separated differences for LC dFC
data <- read.csv('LDA_separated_diff_del_nondel_like_avg_dFC_mtd_lc.csv', header = TRUE)
data_plot <- data.frame(label = label[1:400], SignificantCoefficient = data$XO[1:400])
plot_brain_data_17nets(data_plot, "LDA_separated_diff_del_nondel_like_avg_dFC_mtd_lc_updated")
...
## FC FIGURES - FIGURE 3
```{r}
# PLOTS FOR FC DIFFERENCES IN AAS - FIGURE 3 - Average Timeseries of the hemisphere ascending arousal nuclei first:
schaef_names <- read.csv("/Users/ntaylor/Library/CloudStorage/OneDrive-TheUniversityofSydney(Staff)/Postdoc_Rob/Analysis/Graph_Theory/schaef_400/lm_model/schaef_names.csv", header = TRUE, stringsAsFactors = FALSE)
regions = schaef_names$ROI.Name
label <- schaef_names$ROI.Name
# load in relevant data
setwd('/Users/ntaylor/Library/CloudStorage/OneDrive-TheUniversityofSydney(Staff)/Postdoc_Rob/Analysis/Graph_Theory/schaef_400/csf_regress_redo')
data <- read.csv('sig_diff_nbm_fc_update1.csv',header=FALSE)
# Create a data frame for plotting
data_plot <- data.frame(label = label[1:400], SignificantCoefficient = data$V1[1:400])
plot_brain_data_17nets(data_plot, "sig_diff_nbm_fc_csf4thvent_meantsnuclei")

# FC calculated for each nuclei, then averaged for hemispheres
#data <- read.csv('sig_diff_nbm_fc_whole.csv',header=FALSE)
data <- read.csv('nbM_combined_fc_sig2_diff_csf4thvent.csv',header=FALSE)
#needs to be flipped
data_plot <- data.frame(label = label[1:400], SignificantCoefficient = data$V1[1:400])
plot_brain_data_17nets(data_plot, "sig2_diff_nbm_fc_combined_csf4thvent")

# LC FC differences 
data <- read.csv('lc_combined_fc_sig2_diff_csf4thvent.csv',header=FALSE)
#needs to be flipped
#data <- as.data.frame(matrix(as.numeric(unlist(data)), nrow = 502, ncol = 1))
data_plot <- data.frame(label = label[1:400], SignificantCoefficient = data$V1[1:400])
plot_brain_data_17nets(data_plot, "sig2_diff_lc_fc_combined_csf4thvent")
